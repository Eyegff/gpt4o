...
let isCreatingCode = false; // ตัวแปรเพื่อติดตามสถานะการสร้างโค้ด
...
async function handleEvent(event) {
  ...
  if (event.message.text === '/สร้างโค้ด') {
    isCreatingCode = true; // ตั้งค่าสถานะเป็น true เมื่อเริ่มสร้างโค้ด
    return client.replyMessage(event.replyToken, {
      type: 'text',
      text: 'กรุณาตั้งชื่อโค้ดของคุณ'
    });
  } else if (isCreatingCode) { // หากกำลังสร้างโค้ด
    ...
    return client.replyMessage(event.replyToken, {
      ...
      const expiryTime = Date.now() + (2 * 60 * 60 * 1000); // กำหนดเวลาหมดอายุโค้ด (2 ชั่วโมง)
      ...
      return request(loginOptions, function (loginError, loginResponse) {
        ...
        client.pushMessage(event.source.userId, {
          type: 'text',
          text: 'ล็อกอินสำเร็จ! กำลังสร้างโค้ด...'
        });
